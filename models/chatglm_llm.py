...
# 这里的路径是上传文档的路径
bm25_model = BM25Retrieval("/Users/niulushuai/PycharmProjects/engine-llm/content/test.txt")
...
class ChatGLM(LLM):
    ...
    def sql_chat(self,
             prompt: str, history: List[List[str]] = []):
        response, _ = self.model.chat(
            self.tokenizer,
            prompt,
            history=history[-self.history_len:] if self.history_len > 0 else [],
    ...
    def re_process_prompt(self, prompt):
        query = prompt.split("\"")[1]
        retrievals = bm25_model.retrieval(query, 6)
        new_prompt = f"""基于以下已知信息，简洁和专业的来回答用户的问题，如果无法从中得到答案，请说 "根据已知信息无法回答该问题" 或 "没有提供足够的相关信息"，不允许在答案中添加编造成分，答案请使用中文。
        已知内容:
        {"".join(retrievals)}
